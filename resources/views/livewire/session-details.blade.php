...
@endif
        <form wire:submit.prevent="save" class="mb-4">
            ...
            <button type="submit" class="bg-green-500 hover:bg-green-700  font-bold py-2 px-4 rounded ml-2">Add Session</button>
        </form>
            ...
            @foreach ($session as $sessions)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <button wire:click="edit({{ $sessions->id }})" class="text-indigo-600 hover:text-indigo-900">Edit</button>
                        <button wire:click="delete({{ $sessions->id }})" class="text-red-600 hover:text-red-900 ml-2">Delete</button>
                    </td>
                </tr>
                    ...
                    <tr>
                        ...
                        <td colspan="2">
                            ...
                            <form wire:submit.prevent="update" class="mb-4">
                                <div class="mb-4">
                                    <label for="selectedMonth" class="block text-gray-700 text-sm font-bold mb-2">Month:</label>
                                    <select wire:model="u_month" id="selectedMonth" class="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                        <option value="">Select Month</option>
                                        @foreach ($months as $key => $month)
                                        @selected($month === $u_session->month ? 'disabled' : '')
                                        <option value="{{ $month }}" {{ $month === $u_session->month ? 'disabled' : '' }}>{{ $month }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-4">
                                    <label for="university" class="block text-gray-700 text-sm font-bold mb-2">University Name:</label>
                                    <select wire:model="u_university" id="university" class="shadow appearance-none border rounded py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                                        <option value="">Select University </option>
                                        @foreach($universities as $uni)
                                            <option value="{{ $uni->id }}" {{ $uni->id === $u_session->u_id ? 'disabled':'' }}>{{ $uni->university_name }}</option>
                                        @endforeach
                                    </select>
                                    @error('university') <p class="text-red-500 text-xs italic">{{ $message }}</p> @enderror
                                </div>
                            
                                <button type="submit" class="bg-green-500 hover:bg-green-700  font-bold py-2 px-4 rounded ml-2">Update Session</button>
                            </form>
